...
def sync_to_vector(post):
    """Push post.params into post.vector and refresh cached vary-param indices.

    Since radvel 1.4 all numerics read `post.vector` (values, vary flags,
    mcmcscale, linear), and `post.params` is only a view that radvel refreshes
    *from* the vector. Writes to `post.params` after the Posterior has been
    constructed are therefore silently ignored unless pushed back with this.

    Both `Posterior.list_vary_params` and `Likelihood.list_vary_params` are
    needed: the two cache `vary_params` independently, and `Likelihood.bic()`
    counts free parameters from the likelihood's copy.

    Args:
        post (radvel.Posterior): posterior whose params were just modified

    Returns:
        radvel.Posterior: the same posterior, synced
...
def initialize_post(data, params=None, priors=[], linear=True, decorrs=None):
    ...
    # The gamma/jit assignments above happen after RVLikelihood.__init__ has
    # already built the vector, so push them back in. The analytic-gamma path
    # gates on the linear and vary slots of the vector, not on params.
    sync_to_vector(post)
