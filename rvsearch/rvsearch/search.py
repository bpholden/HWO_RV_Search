...
class Search(object):
    ...
    def trend_test(self):
        ...
        utils.sync_to_vector(self.post)
    ...
    def running_per(self):
        ...
        # rv_drive wants tp/e/w, which are not in the fitting basis. Convert
        # here; radvel <= 1.3.8 left them in post.params as a side effect of
        # MultipanelPlot, which now works on a copy.
        ...
        for n in np.arange(1, self.num_planets+1):
            ...
            for p in planets[planets != n]:
                orbel = [synthparams['per{}'.format(p)].value,
                         synthparams['tp{}'.format(p)].value,
                         synthparams['e{}'.format(p)].value,
                         synthparams['w{}'.format(p)].value,
                         synthparams['k{}'.format(p)].value]
                yres -= radvel.kepler.rv_drive(x, orbel)
            # Make small period grid. Figure out proper spacing.
            per = synthparams['per{}'.format(n)].value
    ...
    def run_search(self, fixed_threshold=None, outdir=None, mkoutdir=True,
        ...
        while run:
            ...
            if self.save_outputs and self.num_planets > 0:
