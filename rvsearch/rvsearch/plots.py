...
class PeriodModelPlot(radvel.plot.orbit_plots.MultipanelPlot):
    ...
    Note:
        Everything except the summary-panel geometry is set up by
        `MultipanelPlot.__init__`, so the residual panel follows radvel's
        current convention: raw residuals against a zero line, *not* the
        pre-1.4 convention of trend-restored residuals against the trend curve.
    ...
    def __init__(self, search, saveplot=None, epoch=2450000, yscale_auto=False,
        ...
        super(PeriodModelPlot, self).__init__(
            search.post, saveplot=saveplot, epoch=epoch,
            yscale_auto=yscale_auto, yscale_sigma=yscale_sigma,
            phase_nrows=phase_nrows, phase_ncols=phase_ncols, uparams=uparams,
            telfmts=telfmts, legend=legend, phase_limits=phase_limits,
            nobin=nobin, phasetext_size=phasetext_size,
            rv_phase_space=rv_phase_space, figwidth=figwidth,
            fit_linewidth=fit_linewidth, set_xlim=set_xlim, text_size=text_size,
            highlight_last=highlight_last, show_rms=show_rms,
        ...
        # MultipanelPlot only passes text_size to rcParams; the summary panels
        # size their own labels relative to it.
        self.text_size = text_size
        self.summary_ncols = summary_ncols  # Number of columns for phase & pers
